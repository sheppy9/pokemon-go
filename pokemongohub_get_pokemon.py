...
def parse_pokemon_data(html_content):
	...
	data['dps'] = float(moveset_article.find_all('div')[5].select_one('div>span:nth-child(2)').text)
	data['tdo'] = float(moveset_article.find_all('div')[6].select_one('div>span:nth-child(2)').text)
	data['score'] = float(moveset_article.find_all('div')[7].select_one('div>span:nth-child(2)').text)
	data['moves_combination'] = [{
		...
		'dps': float(tr.select_one('td:nth-child(4)').text.strip()),
		'tdo': float(tr.select_one('td:nth-child(5)').text.strip()),
		'score': float(tr.select_one('td:nth-child(6)').text.strip())
	...
	mega_boost_pokemon = articles[11]
